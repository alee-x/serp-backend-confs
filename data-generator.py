...
if __name__ == "__main__":
    ...
    fpaths = ['db2', 'mssql', 'postgres', 'minio']
    for path in fpaths:
        pri_string = 'db-startup-scripts/{0}/data/'.format(path)
        path_str = os.path.join(dir, pri_string)
        pri_string = path_str
        wdsddf.to_csv(pri_string + 'wdsd_main.csv', index=False)
        superspelldf.to_csv(pri_string + 'pedw_superspell.csv', index=False)
        spelldf.to_csv(pri_string + 'pedw_spell.csv', index=False)
        episodedf.to_csv(pri_string + 'pedw_episode.csv', index=False)
        wlgpregdf.to_csv(pri_string + 'wlgp_reg.csv', index=False)
        wlgpeventdf.to_csv(pri_string + 'wlgp_event.csv', index=False)