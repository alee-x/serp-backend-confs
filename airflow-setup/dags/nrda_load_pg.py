...
psubdir = currentdir+'/scripts/nrda/data_loading/postgres'

sys.path.append(psubdir)
...
with DAG(dag_id="nrda_load_postgres_pipeline", schedule_interval=None, default_args=default_args, catchup=False, max_active_runs=1, user_defined_macros={
    'json': json
}) as dag:
    ...
    extract_csv_to_load = SparkSubmitOperator(
        ...
        executor_memory="30g",
        executor_cores=9,
        application_args=["stage", "{{ dag_run.conf['project_code'] }}", "{{ ti.xcom_pull(key='ledger_json_file_name') }}", ",", "pg"],
        verbose=False
    )
    ...
    store_ledger_to_file >> extract_csv_to_load >> download_csv_to_load >> apply_sed >> gen_load_table >> gen_base_table >> run_load_load >> is_post_processing_required
    is_post_processing_required >> run_base_load_without_postprocess >> prep_result_legders
    prep_result_legders >> housekeeping >> send_message