...
def gen_load_table(**context):
    ...
    if 'ms_database_schema' in ledger["attributes"]:
        schema_name = "load{0}t".format(ledger["attributes"]["ms_database_schema"].lower())
    ...
    # for item in enc_col_list:
    #     create_table_sql_mssql_str = create_table_sql_mssql_str + "\n--create index {1}_{2}_I1 ON {0}.{1} ({2});".format(schema_name,table_name,item['col_name'])                                    
    ...
    print("using connection: ",db_conn_id)
    print("username: ",mssql_conn.login)
    print("host: ",mssql_conn.host)
