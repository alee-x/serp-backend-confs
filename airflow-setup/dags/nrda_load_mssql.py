...
sys.path.insert(1, '/usr/local/airflow/dags/scripts/nrda')
sys.path.insert(1, '/usr/local/airflow/dags/scripts/nrda/data_loading/mssql')
...
dsubdir = currentdir+'/scripts/nrda/data_loading'

sys.path.append(dsubdir)

msubdir = currentdir+'/scripts/nrda/data_loading/mssql'
...
from gen_load_table import gen_load_table
from run_load_load import run_load_load
from run_xref_load import run_xref_load
from gen_base_table import gen_base_table
from run_base_load import run_base_load
from run_base_load_without_postprocess import run_base_load_without_postprocess
...
with DAG(dag_id="nrda_load_mssql_pipeline", schedule_interval=None, default_args=default_args, catchup=False, max_active_runs=1, user_defined_macros={
    'json': json
}) as dag:
    ...
    extract_csv_to_load = SparkSubmitOperator(
        ...
        executor_memory="30g",
        executor_cores=9,
        application_args=["stage", "{{ dag_run.conf['project_code'] }}", "{{ ti.xcom_pull(key='ledger_json_file_name') }}", "\x01", "mssql"],
        verbose=False
    )
